...
export function Faq() {
  ...
  const filtered = useMemo(() => {
    ...
    return faqEntries.filter((entry) => {
      ...
      const matchesQuery = !q || entry.question.toLowerCase().includes(q) || entry.answer.toLowerCase().includes(q);
            ...
            <div className="flex flex-col gap-4">
              <AnimatePresence mode="popLayout">
                {filtered.length > 0 ? (
                  ...
                  >
                    No questions match &quot;{query}&quot; — try a different search or category.
                  </motion.p>
                )}
              </AnimatePresence>
            </div>
