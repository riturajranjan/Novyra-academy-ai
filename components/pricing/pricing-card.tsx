...
import { CircleCheck, ChevronDown, Sparkles } from "lucide-react";
import {
  motion,
  useMotionTemplate,
  useMotionValue,
...
/** Below this many features, mobile shows a "View all features" toggle
 * instead of the full list — desktop (md+) always shows everything via the
 * responsive classes on the overflow list, regardless of this state. */
const MOBILE_VISIBLE_FEATURES = 7;
...
export function PricingCard({ plan, mode, index }: PricingCardProps) {
  ...
  const visibleFeatures = plan.features.slice(0, MOBILE_VISIBLE_FEATURES);
  const overflowFeatures = plan.features.slice(MOBILE_VISIBLE_FEATURES);
  ...
  return (
    <motion.div
      ...
      <div className="bg-surface/90 relative flex flex-1 flex-col gap-4 rounded-[27px] p-5 md:gap-6 md:p-7">
        <motion.div
          aria-hidden
          className="pointer-events-none absolute inset-0 -z-10 rounded-[27px]"
          style={{ backgroundImage: highlight }}
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-24 rounded-t-[27px] bg-gradient-to-b from-white/10 to-transparent"
        />
        {plan.featured ? (
          <motion.div
            aria-hidden
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-display-lg text-foreground font-semibold">
            {plan.price[mode]}
            <span className="text-title text-foreground-secondary font-medium">
              {plan.priceSuffix[mode]}
            </span>
          </p>
        </div>

        <div className="border-border-subtle flex flex-col gap-3 border-t pt-4 md:pt-5">
          {plan.inheritsFrom ? (
            <p className="text-caption text-foreground-secondary font-semibold tracking-wide uppercase">
          ...
          {overflowFeatures.length > 0 ? (
            <>
              ...
              <button
                ...
                className="text-caption flex items-center gap-1 font-semibold "
                style={{ color: stroke }}>
                {expanded
                  ? "Show less"
                  : `View all features (+${overflowFeatures.length})`}
                <ChevronDown
                  className={cn(
