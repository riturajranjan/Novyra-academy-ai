        ...
        <table className="w-full min-w-[720px] border-collapse">
          <thead>
            <tr className="border-border-subtle border-b">
              <th className="bg-surface sticky left-0 z-10 p-4 text-left text-body-sm font-semibold text-foreground-secondary">
                {t("comparePanel.featureColumnHeader")}
              </th>
              {pricingPlans.map((plan) => (
                <th key={plan.id} className="text-body-sm text-foreground p-4 text-center font-semibold">
          ...
          {compareGroups.map((group) => {
            ...
            return (
              <tbody key={group.id}>
                <tr className="border-border-subtle bg-foreground/[0.02] border-b">
                  <td colSpan={pricingPlans.length + 1} className="p-0">
                    <button
                      type="button"
                      onClick={() => toggleGroup(group.id)}
                      aria-expanded={isOpen}
                      className="text-caption text-foreground-secondary flex w-full items-center justify-between gap-2 px-4 py-2.5 text-left font-semibold tracking-wide uppercase"
                    >
                ...
                {isOpen
                  ? rows.map((row, i) => (
                      <tr
                        key={row.id}
                        className={cn(
                          "border-border-subtle/60 hover:bg-foreground/[0.03] h-12 border-b transition-colors duration-fast last:border-b-0",
                          i % 2 === 1 && "bg-foreground/[0.015]",
                        )}
                      >
                        <td className="bg-surface sticky left-0 z-10 text-body-sm text-foreground p-4 font-medium">
  ...
  return (
    <div className="flex flex-col gap-2 md:hidden">
      {compareGroups.map((group) => {
        ...
        const rows = rowsForGroup(group.rowIds);
        return (
          <div key={group.id} className="border-border-subtle bg-surface/60 overflow-hidden rounded-2xl border backdrop-blur-xl">
            <button
              type="button"
            ...
            >
              <span className="text-body-sm text-foreground font-semibold">{t(`compareGroups.${group.id}.title`)}</span>
              <ChevronDown className={cn("text-foreground-secondary h-4 w-4 transition-transform duration-fast", isOpen && "rotate-180")} aria-hidden />
            </button>
            <AnimatePresence initial={false}>
              {isOpen ? (
                ...
                >
                  <div className="border-border-subtle flex flex-col gap-4 border-t px-4 py-3">
                    {rows.map((row) => (
                      <div key={row.id} className="flex flex-col gap-1.5">
                        <span className="text-body-sm text-foreground font-medium">{t(`compareRows.${row.id}.label`)}</span>
                        <div className="grid grid-cols-2 gap-2">
                          {pricingPlans.map((plan) => (
                            <div
                              key={plan.id}
                              className="border-border-subtle flex items-center justify-between gap-2 rounded-lg border px-2.5 py-1.5"
                            >
                              <span className="text-caption text-foreground-secondary truncate">{t(`plans.${plan.id}.name`)}</span>
                              <Cell value={row.values[plan.id]} color={accentStroke[plan.accent]} rowId={row.id} planId={plan.id} t={t} />
                            </div>
                          ))}
                        </div>
